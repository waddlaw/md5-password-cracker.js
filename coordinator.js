function breakMd5(md5Hash) {
    ...
    for (var i = 0; i < numWorkers; i++) {
      ...
      var worker = new Worker('worker.js');
        workers.push(worker); // Message handler
      ...
      worker.postMessage({ cmd: "setWorkerId", data: i });
        ...
        worker.postMessage({ cmd: "setPassToCrack", data: md5Hash }); // Start worker
      worker.postMessage({ cmd: "performCrack", data: {start: i, hop: numWorkers} });
    }

    status("Searching for password match for hash '"+md5Hash+"'.");
    log("Testing uppercase, lowercase, and numbers."); // Helper functions
